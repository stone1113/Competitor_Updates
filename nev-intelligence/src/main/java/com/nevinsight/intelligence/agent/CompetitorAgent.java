...
public class CompetitorAgent implements ReportAgent<CompetitorAgent.Input, CompetitorAgent.Output> {
    ...
    private static final String EVENTS_BRIEFING_SYSTEM_PROMPT =
            ...
            + "2) 风险提示：判断这些动作对猛士 M817/917 的潜在影响，如价格挤压、产品节奏抢先、声量分流、销量差距、终端转化压力；只保留主要风险判断，避免展开多层解释；没有明显风险也要说明“暂未发现明确负面信号”。\n"
            + "3) 应对策略：给出猛士可执行的响应建议，优先围绕传播重点、终端话术、价格权益、渠道线索、用户反馈、竞品分流监控；输出 2-3 个最关键动作，避免长句堆叠。\n"
            + "\n"
            + "**风格**：分析师口吻，要有判断，不只是罗列。动态总结控制在 120-180 字；风险提示控制在 100-120 字；应对策略控制在 100-120 字。\n"
