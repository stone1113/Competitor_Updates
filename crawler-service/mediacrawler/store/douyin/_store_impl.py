...
class DouyinDbStoreImplement(AbstractStore):
    @staticmethod
    def _cast_bigint_fields(item: Dict, fields: list) -> None:
        """Cast string fields to int for BigInteger columns."""
        for f in fields:
            val = item.get(f)
    ...
    async def store_content(self, content_item: Dict):
        ...
        """
        self._cast_bigint_fields(content_item, ["aweme_id", "create_time"])
    ...
    async def store_comment(self, comment_item: Dict):
        ...
        Args:
            comment_item: comment item dict
        """
        self._cast_bigint_fields(comment_item, ["comment_id", "aweme_id", "create_time"])
