...
async def update_kuaishou_video(video_item: Dict):
    ...
    save_content_item = {
        ...
        "liked_count": str(photo_info.get("realLikeCount") or photo_info.get("likeCount") or ""),
        ...
        "brand_name": getattr(config, 'BRAND_NAME', ''),
    }
    utils.logger.info(
...
async def update_ks_video_comment(video_id: str, comment_item: Dict):
    ...
    save_comment_item = {
        "comment_id": int(comment_id) if comment_id else None,  # BigInteger column
