...
class XiaoHongShuLogin(AbstractLogin):
    ...
    async def check_login_state(self, no_logged_in_session: str) -> bool:
        ...
        try:
            ...
            # Page may be navigating after successful QR scan, ignore content read errors
            pass
    ...
    async def login_by_qrcode(self):
        ...
        if not base64_qrcode_img:
            ...
            for sel in login_selectors:
                try:
                    btn = self.context_page.locator(sel)
                    if await btn.count() > 0:
                        utils.logger.info(f"[XiaoHongShuLogin.login_by_qrcode] 点击登录按钮: {sel}")
                        await btn.first.click(timeout=10000)
                        clicked = True
                        break
                except Exception as e:
                    utils.logger.info(f"[XiaoHongShuLogin.login_by_qrcode] 选择器 {sel} 未匹配: {e}")
                    continue
            ...
            if not base64_qrcode_img:
                utils.logger.error("[XiaoHongShuLogin.login_by_qrcode] 无法找到二维码，请手动登录")
                sys.exit()
