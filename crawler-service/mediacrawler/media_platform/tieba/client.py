...
class BaiduTieBaClient(AbstractApiClient):
    ...
    async def _refresh_proxy_if_expired(self) -> None:
        ...
        Only refreshes when proxy has been activated.
        """
        if self.ip_pool is None or not self._proxy_activated:
            return
    ...
    async def activate_proxy(self) -> None:
        """Activate proxy when anti-crawling is detected."""
        if self.ip_pool is None:
            utils.logger.warning("[BaiduTieBaClient.activate_proxy] No proxy pool available")
            return
        self._proxy_activated = True
        utils.logger.info("[BaiduTieBaClient.activate_proxy] Anti-crawling detected, activating proxy!")
        new_proxy = await self.ip_pool.get_proxy()
        _, self.default_ip_proxy = utils.format_proxy_info(new_proxy)
        utils.logger.info(f"[BaiduTieBaClient.activate_proxy] Now using proxy: {new_proxy.ip}:{new_proxy.port}")
    ...
    async def request(self, method, url, return_ori_content=False, proxy=None, **kwargs) -> Union[str, Any]:
        ...
        try:
            ...
            utils.logger.warning(f"[BaiduTieBaClient.request] Network error: {e}, activating proxy and retrying...")
            await self.activate_proxy()
            response = await asyncio.to_thread(
                self._sync_request,
                method,
                url,
                self.default_ip_proxy,
                **kwargs
            )
