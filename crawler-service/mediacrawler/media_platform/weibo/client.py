...
class WeiboClient(ProxyRefreshMixin):
    ...
    async def request(self, method, url, **kwargs) -> Union[Response, Dict]:
        ...
        try:
            ...
            utils.logger.warning(f"[WeiboClient.request] Network error: {e}, activating proxy and retrying...")
    ...
    async def get_all_notes_by_creator_id(
        ...
        while notes_has_more:
            ...
            # 早停：达到 max_count 立即跳出（max_count<=0 表示不限制 = 兼容旧行为）
            if max_count > 0 and len(result) >= max_count:
                utils.logger.info(f"[WeiboClient.get_all_notes_by_creator] max_count={max_count} reached, stop")
                break
        return result
