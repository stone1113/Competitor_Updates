    ...
    async def request(self, method, url, **kwargs) -> Any:
        ...
        for attempt in range(max_retries):
            try:
                ...
                break
            except (httpx.NetworkError, httpx.ProxyError, httpx.TimeoutException, ssl.SSLError) as e:
                utils.logger.warning(f"[KuaiShouClient.request] Network error (attempt {attempt+1}/{max_retries}): {e}")
                if attempt < max_retries - 1:
                    await self.activate_proxy()
                else:
                    utils.logger.error(f"[KuaiShouClient.request] All {max_retries} attempts failed, returning empty")
                    return {}
    ...
    async def request_rest_v2(self, uri: str, data: dict) -> Dict:
        ...
        for attempt in range(max_retries):
            try:
                ...
                utils.logger.warning(f"[KuaiShouClient.request_rest_v2] Network error (attempt {attempt+1}/{max_retries}): {e}")
                if attempt < max_retries - 1:
                    await self.activate_proxy()
                else:
                    utils.logger.error(f"[KuaiShouClient.request_rest_v2] All {max_retries} attempts failed")
                    raise DataFetchError(f"Network error after {max_retries} retries: {e}")
