            ...
            while (page - start_page + 1) * dy_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                ...
                try:
                    ...
                    posts_res = await self.dy_client.search_info_by_keyword(
                        ...
                        sort_type=SearchSortType(getattr(config, 'DY_SORT_TYPE', 0)),
                    ...
                    utils.logger.error(f"[DouYinCrawler.search] search douyin keyword: {keyword} error: {e}")
                    break
    ...
    async def get_aweme_detail(self, aweme_id: str, semaphore: asyncio.Semaphore) -> Any:
        ...
        async with semaphore:
            try:
                ...
                utils.logger.error(f"[DouYinCrawler.get_aweme_detail] Get aweme detail unexpected error: {ex}")
                return None
    ...
    async def get_comments(self, aweme_id: str, semaphore: asyncio.Semaphore) -> None:
        async with semaphore:
            try:
                ...
                utils.logger.error(f"[DouYinCrawler.get_comments] aweme_id: {aweme_id} get comments unexpected error: {e}")
