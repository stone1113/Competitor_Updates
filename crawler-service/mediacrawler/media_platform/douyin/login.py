...
class DouYinLogin(AbstractLogin):
    ...
    async def begin(self):
        ...
        try:
            ...
            utils.logger.info(f"[DouYinLogin.begin] Page navigating after login, skipping slider check: {e}")
    ...
    async def popup_login_dialog(self):
        ...
        try:
            # check dialog box is auto popup and wait for 120 seconds
            # (give user time to manually pass slider/CAPTCHA verification)
            utils.logger.info("[DouYinLogin.popup_login_dialog] 等待登录弹窗（如有滑块验证请手动完成）...")
            await self.context_page.wait_for_selector(dialog_selector, timeout=1000 * 120)
        except Exception as e:
            utils.logger.error(f"[DouYinLogin.popup_login_dialog] login dialog box does not pop up automatically, error: {e}")
            utils.logger.info("[DouYinLogin.popup_login_dialog] trying to manually click the login button (120s timeout)...")
            login_button_ele = self.context_page.locator("xpath=//p[text() = '登录']")
            await login_button_ele.click(timeout=1000 * 120)
