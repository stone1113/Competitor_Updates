    ...
    async def request(self, method, url, **kwargs):
        ...
        for attempt in range(max_retries):
            try:
                ...
                break
            except (httpx.NetworkError, httpx.ProxyError, httpx.TimeoutException) as e:
                utils.logger.warning(f"[DouYinClient.request] Network error (attempt {attempt+1}/{max_retries}): {e}")
                if attempt < max_retries - 1:
                    await self.activate_proxy()
                else:
                    utils.logger.error(f"[DouYinClient.request] All {max_retries} attempts failed")
