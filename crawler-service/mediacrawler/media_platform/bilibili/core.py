        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            try:
                while (page - start_page + 1) * bili_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                    ...
                    sort_order = getattr(config, 'BILI_SORT_ORDER', '')
                    order = SearchOrderType(sort_order) if sort_order else SearchOrderType.DEFAULT
                ...
                utils.logger.error(f"[BilibiliCrawler.search_by_keywords] Error crawling keyword '{keyword}': {e}, moving to next keyword")
    ...
    async def get_video_info_task(self, aid: int, bvid: str, semaphore: asyncio.Semaphore) -> Optional[Dict]:
        ...
        async with semaphore:
            try:
                ...
                utils.logger.error(f"[BilibiliCrawler.get_video_info_task] Unexpected error for video {bvid or aid}: {ex}")
                return None
    ...
    async def get_video_play_url_task(self, aid: int, cid: int, semaphore: asyncio.Semaphore) -> Union[Dict, None]:
        ...
        async with semaphore:
            try:
                ...
                utils.logger.error(f"[BilibiliCrawler.get_video_play_url_task] Unexpected error: {ex}")
                return None
