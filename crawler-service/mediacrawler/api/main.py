...
async def get_bettafish_keywords():
    """Get keywords from BettaFish project configuration"""
    keywords = []
    # Read from BettaFish's MediaCrawler config
    bettafish_config = os.path.join(
        os.path.dirname(os.path.dirname(os.path.dirname(__file__))),
        "MindSpider", "DeepSentimentCrawling", "MediaCrawler", "config", "base_config.py"
    )
    if os.path.exists(bettafish_config):
        try:
            with open(bettafish_config, "r", encoding="utf-8") as f:
                for line in f:
                    line = line.strip()
                    if line.startswith("KEYWORDS") and "=" in line:
                        # Extract string value: KEYWORDS = "a,b,c"
    ...
    # Fallback: read from current project's config
    if not keywords:
        try:
            import config as crawler_config
            raw = getattr(crawler_config, "KEYWORDS", "")
